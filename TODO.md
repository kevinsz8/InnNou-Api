...
### Fase B: SupplierInvoice + 3-way matching con tolerancias — CONSTRUIDO 2026-07-31

Full detail (diseño completo de los 8 puntos acordados, schema, bugs encontrados+corregidos): `.claude/SupplierInvoiceModule.md`.

Registra facturas de proveedor contra una o varias PO ya `RECEIVED` del mismo Supplier (consolidación multi-PO, precedente Adaco), pre-llenando cantidad/precio desde lo efectivamente recibido (post-rectificación) y comparando contra lo facturado dentro de una tolerancia — sin bloquear el guardado si hay discrepancia (`SupplierInvoiceStatusId = DISCREPANCY`, solo visible/filtrable, sin workflow de aprobación en V1). España exige mantener un "Libro Registro de Facturas Recibidas" con numeración interna secuencial propia del receptor (`FR-{Año}-{5 dígitos}`, mismo patrón atómico que `PurchaseOrderNumberCounters`) — confirmado por investigación.
...
Backend+frontend construidos y verificados por curl+navegador 2026-07-31 (consolidación multi-PO, MATCHED y DISCREPANCY, numeración secuencial, transición de PO a `INVOICED`, adjunto). 4 bugs reales encontrados y corregidos durante la verificación (2 backend: totales no computados en Create/GetByToken; 2 frontend: ediciones de línea descartadas al re-seleccionar PO, mensaje engañoso de "tolerancia no configurada" que en realidad era un 403 Forbidden) — detalle completo en `.claude/SupplierInvoiceModule.md`. **Commiteado y pusheado 2026-07-31 (Api 69e72c3, Web bfb0dcf).**
