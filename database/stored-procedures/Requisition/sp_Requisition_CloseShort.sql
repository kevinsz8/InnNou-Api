...
GO
/* =============================================================
   REQUISITION - CLOSE SHORT
   Only reachable from PARTIALLY_ISSUED — mirrors PurchaseOrder's own
   CLOSED_SHORT exactly: some stock already left the store, the rest never
   will, and RequisitionLines/QuantityRequested is deliberately left
   untouched (the shortfall stays a real, permanent fact rather than being
   rewritten away).
   ============================================================= */
CREATE OR ALTER PROCEDURE dbo.sp_Requisition_CloseShort
(
...
BEGIN
    ...
    SET RequisitionStatusId = (SELECT RequisitionStatusId FROM dbo.RequisitionStatuses WHERE Code = 'CLOSED_SHORT'),
        ClosedShortUtc = SYSUTCDATETIME(),
        ClosedShortBy = @ClosedShortBy,
        ClosedShortReason = @ClosedShortReason,
        LastUpdatedUtc = SYSUTCDATETIME(),
        LastUpdatedBy = @ClosedShortBy
    WHERE RequisitionToken = @RequisitionToken
      AND RequisitionStatusId = (SELECT RequisitionStatusId FROM dbo.RequisitionStatuses WHERE Code = 'PARTIALLY_ISSUED');
