...
CREATE OR ALTER PROCEDURE sp_OrderApprovalStep_Create
    ...
    @ActualFamilyAmount     DECIMAL(18,8),
    @CurrencyCode           VARCHAR(3),
    @ApproverUserId         INT,
    @CreatedBy              VARCHAR(150),
    @TriggeringPurchaseOrderRectificationId INT = NULL
AS
BEGIN
    SET NOCOUNT ON;

    INSERT INTO OrderApprovalSteps
        (OrderApprovalStepToken, OrderId, FamilyId, FamilyCode, Level, ThresholdAmount, ActualFamilyAmount, CurrencyCode, ApproverUserId, OrderApprovalStepStatusId, CreatedBy, TriggeringPurchaseOrderRectificationId)
    VALUES
        (@OrderApprovalStepToken, @OrderId, @FamilyId, @FamilyCode, @Level, @ThresholdAmount, @ActualFamilyAmount, @CurrencyCode, @ApproverUserId, (SELECT OrderApprovalStepStatusId FROM OrderApprovalStepStatuses WHERE Code = 'PENDING'), @CreatedBy, @TriggeringPurchaseOrderRectificationId);
