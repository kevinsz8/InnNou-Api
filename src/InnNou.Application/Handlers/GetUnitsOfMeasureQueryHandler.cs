        ...
        : IRequestHandler<GetUnitsOfMeasureQueryRequest, ApiResponse<GetUnitsOfMeasureQueryResponse>>
    {
        public async Task<ApiResponse<GetUnitsOfMeasureQueryResponse>> Handle(GetUnitsOfMeasureQueryRequest request, CancellationToken cancellationToken)
        {
            ...
            if (request.UnitTypeToken.HasValue)
            {
                ...
                if (unitType is null)
                    return ApiResponse<GetUnitsOfMeasureQueryResponse>.FailureResponse(ErrorCodes.UnitTypeNotFound, "Unit type not found.", 404);
                unitTypeId = unitType.UnitTypeId;
            }

            var result = await unitOfMeasureService.GetPagedAsync(request.PageNumber, request.PageSize, unitTypeId, request.IncludeInactive, cancellationToken);
