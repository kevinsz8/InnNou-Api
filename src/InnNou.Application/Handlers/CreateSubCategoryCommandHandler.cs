...
namespace InnNou.Application.Handlers
{
        ...
        : IRequestHandler<CreateSubCategoryCommandRequest, ApiResponse<CreateSubCategoryCommandResponse>>
    {
        public async Task<ApiResponse<CreateSubCategoryCommandResponse>> Handle(CreateSubCategoryCommandRequest request, CancellationToken cancellationToken)
        {
            ...
            if (category is null)
                return ApiResponse<CreateSubCategoryCommandResponse>.FailureResponse("CATEGORY_NOT_FOUND", "Category not found.", 404);

            if (await subCategoryService.ExistsByCodeAsync(request.Code, category.CategoryId, cancellationToken))
                return ApiResponse<CreateSubCategoryCommandResponse>.FailureResponse("SUB_CATEGORY_CODE_EXISTS", "A sub-category with this code already exists in the category.", 409);
            ...
            if (result is null)
                return ApiResponse<CreateSubCategoryCommandResponse>.FailureResponse("SUB_CATEGORY_CREATE_FAILED", "Sub-category could not be created.", 500);
