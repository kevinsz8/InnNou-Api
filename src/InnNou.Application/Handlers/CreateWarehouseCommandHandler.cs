        ...
        : IRequestHandler<CreateWarehouseCommandRequest, ApiResponse<CreateWarehouseCommandResponse>>
    {
        public async Task<ApiResponse<CreateWarehouseCommandResponse>> Handle(CreateWarehouseCommandRequest request, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.PurposeCode) && !WarehousePurposeCodes.IsValid(request.PurposeCode))
                return ApiResponse<CreateWarehouseCommandResponse>.FailureResponse(ErrorCodes.WarehouseInvalidPurpose, "PurposeCode is not one of the supported values.", 400);
